...
layui.define(['laypage', 'fly', 'element', 'flow'], function(exports){
  ...
  gather.mine = function(index, type, url){
    ...
    var page = function(now){
      var curr = now || 1;
      if(gather.minelog[type + '-companies-' + curr]){
        view(gather.minelog[type + '-companies-' + curr]);
      } else {
        //我收藏的帖
        if(type === 'collection'){
          ...
          fly.json(url, {}, function(res){
            ...
            render(curr)
            gather.minelog['collect-companies-' + curr] = res;
          ...
          fly.json('/api/'+ type +'/', {
            ...
            view(res);
            gather.minelog['mine-jie-companies-' + curr] = res;
            now || laypage.render({
              elem: 'LAY_page'
              ,count: res.count
              ,curr: curr
