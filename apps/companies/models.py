...
class Company(models.Model):
    ...
    # author = models.ForeignKey('Author', null=True, blank=True, verbose_name='发布者', on_delete=models.CASCADE, related_name='company_author')
    ...
    def get_absolute_url(self):
        return reverse('companies:company_detail', kwargs={'company_id': self.id})
    ...
    def next_company(self):
        # 下一篇
        return company.objects.filter(id__gt=self.id, status='p').order_by('id').first()

    @cache_decorator(expiration=60 * 100)
    def prev_company(self):
        # 前一篇
        return company.objects.filter(id__lt=self.id, status='p').first()
...
class Book(models.Model):
    ...
    书籍
    """
    company = models.OneToOneField('company', on_delete=models.CASCADE, default=None)  # 非常关键的一对一关联！
...
class CompanyUser(models.Model):
    ...
    class Meta:
        ...
        # unique_together = (("company", "user"),)  # 设置联合主键
        # permissions = (
...
class SiteSettings(models.Model):
    ...
    company_sub_length = models.IntegerField("文章摘要长度", default=300)
    sidebar_company_count = models.IntegerField("侧边栏文章数目", default=10)
