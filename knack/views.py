...
def knack_support(request):
    ...
    if request.method == 'POST':
        ...
        try:
            knack = models.Knack.objects.get(id=knack_id)
            author_id = knack.author_id
        except Exception as e:
            print(e)
            knack = None
            author_id = None
        if request.user.id != author_id:
            if action == 'support':
                try:
                    knack.support = int(knack.support) + 1
                    knack.save()
                    models.KnackUser.objects.update_or_create(knack_id=knack_id, user_id=request.user.id, defaults={'support': 1})
                    return HttpResponse('{"status":"success"}', content_type='application/json')
                except Exception as e:
                    print(e)
                    return HttpResponse('{"status":"fail"}', content_type='application/json')
                ...
                try:
                    knack.support = int(knack.support) - 1
                    knack.save()
                    models.KnackUser.objects.update_or_create(knack_id=knack_id, user_id=request.user.id, defaults={'support': 0})
                    return HttpResponse('{"status":"success"}', content_type='application/json')
                except Exception as e:
                    print(e)
                    return HttpResponse('{"status":"fail"}', content_type='application/json')
